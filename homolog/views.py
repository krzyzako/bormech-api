...
def index(request):
    context = {
        "edit_homolog" : "active",
        "count" : Rodzaj.objects.all().count(),
        "aprovals":Rodzaj.objects.all()
        }
    return render(request, "homolog_index.html",context)
...
def testy(request):
    ...
    return render(request, "testy.html",context)
...
def add(request):  
    if request.method == "POST":  
        ...
        form = RodzajForm()
    checks = Rodzaj.typ
    return render(request,'homolog_add.html',{'form':form,"wybor":checks,"add_homolog" : "active"})  


@login_required(login_url="/login/")
def edit(request, id):  
    tank = Rodzaj.typ
    rodzaj = Rodzaj.objects.get(id=id)
    choise = Rodzaj.typ  
    return render(request,'homolog_edit.html', {'rodzaj':rodzaj,"homolog" : "active",'tank':tank ,'wybor':choise})
...
def update(request, id):  
    rodzaj = Rodzaj.objects.get(id=id)  
    form = RodzajForm(request.POST, instance = rodzaj) 
    print(form)
    print(form.is_valid())
    if form.is_valid():  
        form.save()  
        return redirect("/")
    return render(request, 'homolog_edit.html', {'rodzaj': rodzaj,"homolog" : "active"})  
